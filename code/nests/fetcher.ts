...
const Safe: Set<String>=new Set<String>(["GET", "HEAD", "OPTIONS", "TRACE"]);
		...
		return new Promise(resolve => fetcher(request)
			...
			.then(response => {
				...
				if ( !response.ok && !response.bodyUsed ) { // error response not handled elsewhere
					...
					alert(`;-( Error ${response.status} ${response.statusText}`); // !!! provide customization hook

					response.text().then(text => text && console.error(text));
