...
import { immutable } from "../Core/src/index.js";
import { resolve } from "@metreeca/data/contexts/fetcher";
import { KeyboardEvent } from "react";


/**
 * Default auto-submit delay (ms)
 */
...
export function keys<T=Element>(options: {
	[key: string]: (e: KeyboardEvent<T>) => void | boolean
}): typeof options[string] {

	return e => {

		const option=options[e.key];
		const processed=(option !== undefined && option(e) !== false);
